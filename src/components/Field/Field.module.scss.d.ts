// This file is auto-generated. Do not edit manually.
declare const styles: {
  readonly "field": string;
  readonly "fieldGroup": string;
  readonly "infoButton": string;
  readonly "layoutHorizontal": string;
  readonly "layoutVertical": string;
  readonly "noLabel": string;
  readonly "fieldContent": string;
  readonly "disabled": string;
  readonly "fieldLabel": string;
  readonly "wrapNormal": string;
  readonly "wrapBreakWord": string;
  readonly "wrapNowrap": string;
  readonly "labelText": string;
  readonly "requiredIndicator": string;
  readonly "fieldInput": string;
  readonly "fieldDescription": string;
  readonly "variantHelp": string;
  readonly "variantInfo": string;
  readonly "variantWarning": string;
  readonly "fieldError": string;
  readonly "animationSlide": string;
  readonly "enter": string;
  readonly "enterActive": string;
  readonly "exit": string;
  readonly "exitActive": string;
  readonly "animationFade": string;
  readonly "positionInline": string;
  readonly "errorIcon": string;
  readonly "spacingCompact": string;
  readonly "spacingNormal": string;
  readonly "spacingRelaxed": string;
  readonly "customInput": string;
  readonly "error": string;
  readonly "module": string;
  readonly "css": string;
  readonly "map": string;
};
export default styles;
